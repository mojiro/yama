...
"""Yama: Several validation functions in one module.
<ansible.module_utils.remote_management.yama.valid>"""
...
def hastuple(data):
    """It validates that given input is a tuple with content.

    :param data: (str) Input tuple.
    :return: (bool) True or False.
    """
    if not data:
        return False
...
def ispkey(data):
    ...
    :param data: (str) Private key.
    :return: (bool) True or False.
    """
    if not hasstring(data):
        return False
    regex = re.compile(r'-----BEGIN RSA PRIVATE KEY-----(\r?\n[0-9A-Za-z+\/]+)'
                       r'+[=]{0,3}\r?\n-----END RSA PRIVATE KEY-----')
    if regex.match(data):
        return True
    return False
...
def isuuid(data):
    ...
    if not hasstring(data):
        return False
    regex = re.compile(r'[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b'
                       r'[0-9a-f]{12}')
    if regex.match(data):
        return True
    return False
